...
class NotificationManager {
    ...
    async sendNotification() {
        try {
            ...
            const clickUrl = document.getElementById('click_url').value;
            ...
            let query = this.supabase
                ...
                .select('*')
                .eq('status', 'active')
                .is('unsubscribed_at', null); // Only get subscribers who haven't unsubscribed

            console.log('Initial query for active subscribers');

            // Apply country targeting
            if (!this.selectedCountries.has('ALL')) {
                const countries = Array.from(this.selectedCountries).map(country => country.trim());
                console.log('Targeting specific countries:', countries);
                // Use ilike for case-insensitive matching
                const countryFilter = countries.map(country => `country.ilike.${country}`);
                query = query.or(countryFilter.join(','));
            } else {
                console.log('Targeting ALL countries');
            }

            // Apply device targeting
            if (this.selectedDevice !== 'all') {
                console.log('Targeting specific device:', this.selectedDevice);
                query = query.eq('device_type', this.selectedDevice.toLowerCase());
            } else {
            ...
            if (subscribersError) {
                console.error('Error querying subscribers:', subscribersError);
            ...
            // Filter out subscribers with obviously invalid tokens
            ...
            if (validSubscribers.length === 0) {
                alert('No subscribers with valid tokens found. They may need to resubscribe to notifications.');
                return;
            }

            console.log(`Found ${validSubscribers.length} subscribers with valid tokens`);

            // Send notifications through notification server
            const serverUrl = 'https://notification-server-f0so.onrender.com';
            ...
            try {
                ...
                if (response.ok) {
                    let message = `Notification sent to ${data.sent} devices.`;
                    if (data.failed > 0) {
                        ...
                        if (data.errors && data.errors.length > 0) {
                            ...
                            const invalidTokenErrors = data.errors.filter(error => 
                                error.code === 'messaging/invalid-registration-token' ||
                                error.code === 'messaging/registration-token-not-registered'
                            );

                            if (invalidTokenErrors.length > 0) {
                                message += ` - these subscribers may need to resubscribe to notifications`;
                                ...
                                try {
                                    ...
                                    if (currentToken !== newToken) {
                                        ...
                                        await this.updateSubscriberToken(newToken);
    ...
    async updateSubscriberToken(newToken) {
        try {
            // Get the old token from the database
            ...
            console.error('Error updating subscriber token:', error);
            throw error;
        }
    }
    ...
    formatCampaign(campaign) {
        return {
            ...
            target_device: campaign.target_device,
